...
class KategoriRumah : AppCompatActivity(), PermissionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mFirebaseDatabase = mFirebaseInstance.getReference("Rumah")
                ...
                if (filePath != null) {
                    ...
                    ref.putFile(filePath)
                        .addOnSuccessListener {
                            ...
                            Toast.makeText(this@KategoriRumah, "Uploaded", Toast.LENGTH_SHORT).show()

                            ref.downloadUrl.addOnSuccessListener {
                                preferences.setValues("linkrumah", it.toString())
                            }
                        }
                        .addOnFailureListener { e ->
                            progressDialog.dismiss()
                            Toast.makeText(this@KategoriRumah, "Failed " + e.message, Toast.LENGTH_SHORT)
                                .show()
                        }
        ...
        sell_jual.setOnClickListener {
            if (statusAdd){
                ...
                var sImage = preferences.getValues("linkrumah")
    ...
    private fun saveProject(sNama: String?, sHarga: String?, sDeskripsi: String?, sImage: String?) {
        ...
        rumah.nama = sNama
        rumah.harga = sHarga
        rumah.deskripsi = sDeskripsi
        rumah.images = sImage

        if (sNama != null) {
            checkingUsername(sNama, rumah)
        }
    }

    private fun checkingUsername(iTitle: String, data: Rumah) {
        mFirebaseDatabase.child(iTitle).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {

                val kat = dataSnapshot.getValue(Rumah::class.java)
                if (kat == null) {
                    ...
                    val intent = Intent(this@KategoriRumah,
                        Posted::class.java)
                    startActivity(intent)

                } else {
                    Toast.makeText(this@KategoriRumah, "Nama Sudah Digunakan", Toast.LENGTH_LONG).show()

                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@KategoriRumah, ""+error.message, Toast.LENGTH_LONG).show()
            }
        })
    }
